/*
 *  drivers/cpufreq/cpufreq_intelondemand.c
 *
...
#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_INTELONDEMAND
static
#endif
struct cpufreq_governor cpufreq_gov_intelondemand = {
       .name                   = "intelondemand",
...
static struct workqueue_struct	*kintelondemand_wq;
...
static void intelondemand_powersave_bias_init_cpu(int cpu)
{
...
static void intelondemand_powersave_bias_init(void)
{
	int i;
	for_each_online_cpu(i) {
		intelondemand_powersave_bias_init_cpu(i);
	}
}
...
{
	printk_once(KERN_INFO "CPUFREQ: intelondemand sampling_rate_max "
...
/* cpufreq_intelondemand Governor Tunables */
...
#define DEPRECATION_MSG(file_name)					\
	printk_once(KERN_INFO "CPUFREQ: Per core intelondemand sysfs "	\
				    ...
				    const char *buf, size_t count)
{
	...
	intelondemand_powersave_bias_init();
	mutex_unlock(&dbs_mutex);
...
static void dbs_check_cpu(struct cpu_dbs_info_s *this_dbs_info)
{
	...
	for_each_cpu(j, policy->cpus) {
		...
		/*
		 * For the purpose of intelondemand, waiting for disk IO is an
...
static void do_dbs_timer(struct work_struct *work)
{
	...
	queue_delayed_work_on(cpu, kintelondemand_wq, &dbs_info->work, delay);
...
static inline void dbs_timer_init(struct cpu_dbs_info_s *dbs_info)
{
	...
	queue_delayed_work_on(dbs_info->cpu, kintelondemand_wq, &dbs_info->work,
		delay);
}
				   ...
				   unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
		...
		intelondemand_powersave_bias_init_cpu(cpu);
		/*
...
static int __init cpufreq_gov_dbs_init(void)
{
	...
	kintelondemand_wq = create_workqueue("kintelondemand");
	if (!kintelondemand_wq) {
		printk(KERN_ERR "Creation of kintelondemand failed\n");
		return -EFAULT;
	}
	err = cpufreq_register_governor(&cpufreq_gov_intelondemand);
	if (err)
		destroy_workqueue(kintelondemand_wq);
...
{
	cpufreq_unregister_governor(&cpufreq_gov_intelondemand);
	destroy_workqueue(kintelondemand_wq);
	destroy_dbs_info();
}
...
MODULE_DESCRIPTION("'cpufreq_intelondemand' - A dynamic cpufreq governor for "
	"Low Latency Frequency Transition capable processors");
MODULE_LICENSE("GPL");

#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_INTELONDEMAND
